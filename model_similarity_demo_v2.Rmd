...
metadata <- read.csv("./metadata_categorized.csv") %>%
  dplyr::rowwise() %>%
  dplyr::mutate(hybrid =  ifelse(sum(stats,agent_based,
                                     compartmental,machine_learning)>1, TRUE,FALSE)) 
# manual change 
...
```{r}
library(ggdendro)
recent_dmeta$model_type <- ifelse(recent_dmeta$ensemble, 
                                  "ensemble", 
                                  ifelse(recent_dmeta$hybrid,
                                         "hybrid",
                                         ifelse(recent_dmeta$stats,
                                                "statistical",
                                                ifelse(recent_dmeta$agent_based,
                                                       "agent-based",
                                                       ifelse(recent_dmeta$compartmental,
                                                              "compartmental",
                                                              "machine learning")))))
short_meta <- recent_dmeta[,c(3,ncol(recent_dmeta))]
for(j in 1:5){
for(i in 1:4){
    hclust_dat <- hclust(as.dist(get(paste0("d_frame_loc",loc_name$location[j]))[[i]]), 
                       method = "ward.D", members = NULL)
    ddata <- label(dendro_data(as.dendrogram(hclust_dat))) %>%
      left_join(short_meta,by=c("label"="model_abbr"))
    set1<-c("red","blue","purple","orange","pink","green")
    color_set <- ifelse(ddata$model_type == "ensemble", 
                        set1[1], 
                        ifelse(ddata$model_type == "hybrid",
                               set1[2],
                               ifelse(ddata$model_type == "stats",
                                      set1[3],
                                      ifelse(ddata$model_type =="agent_based",
                                             set1[4],
                                             ifelse(ddata$model_type=="compartmental",
                                                    set1[5],
                                                    set1[6])))))
    ...
    ggdendrogram(
      ...
      theme(axis.text.x = element_text(size=5,colour=color_set),
...
for(loc in loc_name$location){
  tmp <- approx_cd_list[[2]] %>% 
    dplyr::filter(location==loc) 
  tmp_list <-  lapply(1:4, function(x) cd_matrix(tmp,x))
  assign(paste0("c_frame_loc",loc), tmp_list)
}
...
for(loc in loc_name$location){
    ...
    assign(paste0("p_",loc),
      distance_heatmap(tmp,
                       paste0(loc_name$abbreviation[which(loc_name$location==loc)],
                                "- Mean Approx. CD - Inc Case Forecasts by Horizon"),recent_cmeta))
      tmp <-total_frame %>%
          dplyr::filter(location==loc) %>%
          dplyr::group_by(horizon,target_end_date) %>%
          dplyr::filter(model_1 =="COVIDhub-ensemble",
      ...
      assign(paste0("t_",loc),
             scatter(tmp,
            paste0("Approx. CD from COVIDhub-ensemble Over Time  - ",
                loc_name$abbreviation[which(loc_name$location==loc)]),recent_cmeta )
    )
}
```
...
```{r}
recent_cmeta$model_type <- ifelse(recent_cmeta$ensemble, 
                                  "ensemble", 
                                  ifelse(recent_cmeta$hybrid,
                                         "hybrid",
                                         ifelse(recent_cmeta$stats,
                                                "statistical",
                                                ifelse(recent_cmeta$agent_based,
                                                       "agent-based",
                                                       ifelse(recent_cmeta$compartmental,
                                                              "compartmental",
                                                              "machine learning")))))
short_meta <- recent_cmeta[,c(3,ncol(recent_cmeta))]

for(j in 1:5){
for(i in 1:4){
    hclust_dat <- hclust(as.dist(get(paste0("c_frame_loc",loc_name$location[j]))[[i]]), 
                       method = "ward.D", members = NULL)
    ddata <- label(dendro_data(as.dendrogram(hclust_dat))) %>%
      left_join(short_meta,by=c("label"="model_abbr"))
    set1<-c("red","blue","purple","orange","pink","green")
    ...
    assign(paste0("dp_",i),
    ggdendrogram(
      hclust(as.dist(get(paste0("c_frame_loc",loc_name$location[j]))[[i]]), method = "ward.D", members = NULL)
      ,size = 2) +
      labs(title=paste0("Dendrogram - ",i, " wk ahead inc case - ",loc_name$abbreviation[j]))+
      xlab("") +
      ylab("Mean Cramer's Distance") +
      theme(axis.text.x = element_text(size=5,colour = color_set),
            axis.text.y = element_text(size=7),
