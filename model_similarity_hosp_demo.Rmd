...
hosp_pt_high <- map_dfr(date_range,
                 function(x) {
                   covidHubUtils::load_latest_forecasts(last_forecast_date = x,
                                                        ...
                                                        # pick one
                                                        locations = c("06", "12", "13", "36", "48"),
                                                        types = "point",
                                                        targets = paste0(1:28," day ahead inc hosp"),
                                                        source = "zoltar")
                   })
  
hosp_pt_high2 <- hosp_pt_high %>% 
  select(-c(quantile, location, 11:16)) %>%
#  dplyr::filter(weekdays(`target_end_date`) == weekdays(as.Date("2021-06-10"))) %>%
...
ggplot(filter(hosp_pt_high2, model == "LANL-GrowthRate")) + 
...
ggplot(filter(hosp_pt_high2, model == "COVIDhub-ensemble")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  geom_vline(aes(xintercept = as.Date("2021-02-13"), color = "red")) +
  labs(title = "COVIDhub-ensemble Forecasts")

ggplot(filter(hosp_pt_high2, model == "GT-DeepCOVID")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  labs(title = "GT-DeepCOVID Forecasts")

ggplot(filter(hosp_pt_high2, model == "JHUAPL-Bucky")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
...
hosp_pt_low <- map_dfr(date_range,
                 function(x) {
                   covidHubUtils::load_latest_forecasts(last_forecast_date = x,
                                                        forecast_date_window_size=6,
                                                        # pick one
                                                        locations = c("02", "15", "38", "50", "56"),
                                                        types = "point",
                                                        targets = paste0(1:28," day ahead inc hosp"),
                                                        source = "zoltar")
                   })
  
hosp_pt_low2 <- hosp_pt_low %>% 
  select(-c(quantile, location, 11:16)) %>%
#  dplyr::filter(weekdays(`target_end_date`) == weekdays(as.Date("2021-06-10"))) %>%
  dplyr::mutate(horizon_week = case_when(
    horizon %in% 1:7 ~ 1,
...
ggplot(filter(hosp_pt_low2, model == "LANL-GrowthRate")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  geom_vline(aes(xintercept = as.Date("2021-05-22"), color = "red")) +
  labs(title = "LANL-GrowthRate Forecasts")

ggplot(filter(hosp_pt_low2, model == "COVIDhub-ensemble")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  geom_vline(aes(xintercept = as.Date("2021-02-13"), color = "red")) +
  labs(title = "COVIDhub-ensemble Forecasts")

ggplot(filter(hosp_pt_low2, model == "GT-DeepCOVID")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  labs(title = "GT-DeepCOVID Forecasts")

ggplot(filter(hosp_pt_low2, model == "JHUAPL-Bucky")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "JHUAPL-Bucky Forecasts")
...
# plot truth data
ggplot(hosp_truth_forecast_ca_ny, aes(x = target_end_date, y = value, color = model, shape = location)) + 
      geom_point(alpha=0.6,size=0.8) + 
      geom_line() +
