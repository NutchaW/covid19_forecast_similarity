...
ggplot(hosp_truth_us, aes(x = target_end_date, y = value)) + 
      geom_point(alpha=0.6,size=0.8) + 
      geom_line() +
...
lubridate::wday(as.Date("2021-01-31"))

h_forecast <- load_latest_forecasts(
  locations = "US",
  targets = paste(1:31, "day ahead inc hosp"),
  last_forecast_date = as.Date("2021-01-31") ,
  forecast_date_window_size = 6,
  source = "zoltar")

recent <- load_latest_forecasts(
  locations = "US",
  targets = paste(1:31, "day ahead inc hosp"),
  last_forecast_date = as.Date("2021-05-29") ,
  forecast_date_window_size = 6,
  source = "zoltar")

# plot 5 inc hosp models + truth data
plot1 <- plot_forecasts(filter(h_forecast, (model %in% h_meta)), 
                     ...
                     intervals=NULL) + 
  scale_x_date(name=NULL, date_breaks = "1 month", date_labels = "%b '%y", limits=c(as.Date(NA), as.Date("2021-01-31")+32), expand = expansion(mult=c(0.02,0.02))) + 
...
# zoom-in
  ...
  scale_x_date(name=NULL, 
               date_breaks = "1 month", 
               date_labels = "%b '%y", 
               limits = c(as.Date("2021-01-31"), as.Date("2021-02-28"))) 

 
# plot all models
plot2 <- plot_forecasts(filter(h_forecast, (model %in% unique(wide_frame_hosp_high$model))), 
                     target_variable = "inc hosp", 
                     truth_source="HealthData", 
                     subtitle = "none", 
                     title = "none",
...
recent_plot <- plot_forecasts(filter(recent, (model %in% unique(wide_frame_hosp_high$model))), 
                     target_variable = "inc hosp", 
                     truth_source="HealthData", 
                     subtitle = "none", 
                     title = "none",
                     ...
                     fill_by_model = TRUE, fill_transparency = .3,
                     intervals=NULL) + 
  scale_x_date(name=NULL, date_breaks = "1 month", date_labels = "%b '%y", limits=c(as.Date(NA), as.Date("2021-05-29")+32), expand = expansion(mult=c(0.02,0.02))) + 
  theme(axis.ticks.length.x = unit(0.5, "cm"),  
        axis.text.x = element_text(vjust = 7, hjust = -0.2),
        plot.title = element_blank())  
...
recent_plot +
  ...
  scale_x_date(name=NULL, 
               date_breaks = "1 month", 
               date_labels = "%b '%y", 
               limits = c(as.Date("2021-05-29"), as.Date("2021-06-28"))) 


###### 
plot2_select <- plot_forecasts(filter(h_forecast, (model %in% c("LANL-GrowthRate", 
                                                                "COVIDhub-ensemble", 
                                                                "GT-DeepCOVID"))), 
                     target_variable = "inc hosp", 
                     truth_source="HealthData", 
...
recent_plot_select <- plot_forecasts(filter(recent, (model %in% c("LANL-GrowthRate", 
                                                                "COVIDhub-ensemble", 
#                                                                "GT-DeepCOVID",
#                                                                "JHUAPL-Gecko",
...
The truth data has a cyclic pattern that indicates that there may be a day of the week effect such that incident hospitalizations increase during the week but fall on the weekend. However, none of the five models included seem to include day of the week effects.  
