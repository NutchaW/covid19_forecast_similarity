...
wide_frame_hosp_high$target_end_date <- parse_date_time(wide_frame_hosp_high$target_end_date, orders = "ymd")
...
wide_frame_hosp_low <- read.csv("./data/quantile_frame_hosp_bottom.csv") %>%
  dplyr::filter(!(model %in% c("CU-nochange","CU-scenario_high","CU-scenario_low","CU-scenario_mid")))
wide_frame_hosp_low$target_end_date <- parse_date_time(wide_frame_hosp_low$target_end_date, orders = "ymd")
...
We notice that Google_Harvard-CPF has much higher equal and unequal Cramer's Distance values compared to the other models. Its difference between its equal and unequal Cramer Distances is also higher than the other models, but not by the same striking difference. The next model with a large difference is Covid19Sim-Simulator.
...
hosp_pt_high <- hosp_pt_high %>%
  ...
  mutate(cum_hosp = sum(value)) %>%
  distinct(cum_hosp, .keep_all = TRUE) %>%
  #filter(model %in% c("LANL-GrowthRate", "COVIDhub-ensemble", "GT-DeepCOVID", "JHUAPL-Bucky", "Google_Harvard-CPF")) %>%
  filter(horizon_week == 1) %>%
  mutate(day_type = ifelse(wday(target_end_date) %in% 2:6, "weekday", "weekend"))

hosp_pt_high$target_end_date <- parse_date_time(hosp_pt_high$target_end_date, orders = "ymd")
hosp_pt_high$day_type <- as.factor(hosp_pt_high$day_type)

# ggplot(hosp_pt_high) + 
#   geom_point(aes(x = target_end_date, y = cum_hosp, color = model, shape = day_type, alpha = 0.5)) +
#   geom_line(aes(x = target_end_date, y = cum_hosp, color = model)) +
#   facet_wrap(vars(model))

ggplot(filter(hosp_pt_high, model == "LANL-GrowthRate")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
#  geom_vline(aes(xintercept = as.Date("2021-05-22"), color = "red")) +
  labs(title = "LANL-GrowthRate Forecasts") 


ggplot(filter(hosp_pt_high, model == "COVIDhub-ensemble")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-13"), color = "red")) +
  labs(title = "COVIDhub-ensemble Forecasts")

ggplot(filter(hosp_pt_high, model == "GT-DeepCOVID")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  labs(title = "GT-DeepCOVID Forecasts")

ggplot(filter(hosp_pt_high, model == "JHUAPL-Bucky")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "JHUAPL-Bucky Forecasts")

ggplot(filter(hosp_pt_high, model == "Google_Harvard-CPF")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "Google_Harvard-CPF")

ggplot(filter(hosp_pt_high, model == "Karlen-pypm")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "Karlen-pypm ")

ggplot(filter(hosp_pt_high, model == "JHUAPL-SLPHospEns")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  #geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "JHUAPL-SLPHospEns")  
...
hosp_pt_low$target_end_date <- parse_date_time(hosp_pt_low$target_end_date, orders = "ymd")
...
ggplot(filter(hosp_pt_low, model == "COVIDhub-ensemble")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-13"), color = "red")) +
  labs(title = "COVIDhub-ensemble Forecasts")

ggplot(filter(hosp_pt_low, model == "GT-DeepCOVID")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  labs(title = "GT-DeepCOVID Forecasts")

ggplot(filter(hosp_pt_low, model == "JHUAPL-Bucky")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "JHUAPL-Bucky Forecasts")

ggplot(filter(hosp_pt_low, model == "Google_Harvard-CPF")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "Google_Harvard-CPF")

ggplot(filter(hosp_pt_low, model == "Karlen-pypm")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "Karlen-pypm ")

ggplot(filter(hosp_pt_low, model == "JHUAPL-SLPHospEns")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
  #geom_vline(aes(xintercept = as.Date("2021-02-27"), color = "red")) +
  labs(title = "JHUAPL-SLPHospEns")  
...
hosp_pt_ca_ny <- hosp_pt_ca_ny %>% 
  ...
  filter(target_end_date >= as.Date("2021-01-07"), target_end_date <= as.Date("2021-06-10"))  %>%
...
Overall, it seems that Google_Harvard-CPF is consistently the most dissimilar from the other models by a substantial amount, followed by Covid19Sim-Simulator, across almost all horizons for both high-count and low count regions. 
