...
dr <- seq.Date(from = as.Date("2021-01-14"), to = as.Date("2021-06-10"), by = "week")

forecasts_short <- filter(wide_frame_hosp_high, 
                               target_end_date >= as.Date("2021-01-28"), # gives 8 models (single CU)
                               target_end_date <= as.Date("2021-06-10"))

f_short <- frame_format2(forecasts_short)
```
...
cd_diff_high <- total_frame_high %>%
  dplyr::group_by(horizon,target_end_date) %>%
  dplyr::filter(model_1 =="JHUAPL-Bucky",
...
ggplot(cd_diff_high, aes(x=target_end_date, y=mean_approx_cd)) + 
   #   geom_point(alpha=0.6,size=0.8) + 
      geom_line(alpha=0.4) +
      ggtitle("Approx CD over Time - Covid19Sim-Simulator and JHUAPL-Bucky") +
      ylab("Approx. CD") +
      xlab("Forecast End Date") +
...
ggplot(filter(hosp_pt_low, model == "LANL-GrowthRate")) + 
  geom_point(aes(x = target_end_date, y = cum_hosp, shape = day_type)) +
  geom_line(aes(x = target_end_date, y = cum_hosp)) +
 # geom_vline(aes(xintercept = as.Date("2021-05-22"), color = "red")) +
  labs(title = "LANL-GrowthRate Forecasts")
