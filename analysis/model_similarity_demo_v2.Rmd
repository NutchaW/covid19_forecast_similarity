...
&\approx\sum^{2K-1}_{j=1}\frac{\{\hat{F}(q_j)-\hat{G}(q_j)\}^2+\{\hat{F}(q_{j+1})-\hat{G}(q_{j+1})\}^2}{2}(q_{j+1}-q_{j})\label{eq:trap}
\end{align}
...
\tau^F_j=
\begin{cases}
I(q_1=q_1^F)\times \tau_{q_1}^F\hspace{6cm}\text{for }j=1\\
...
\tau^G_j=
\begin{cases}
I(q_1=q_1^G)\times \tau_{q_1}^G\hspace{6cm}\text{for }j=1\\
...
<!---a lot of ideas, look at weeks where model really differ relative to one another or the ensemble, see how the ensemble perform that week or check scores/forecast skill for predictability, does ensemble outperform when there is disagreement?--->
...
```{r}
tab1 <- rbind(short_meta,short_meta_low) %>%
  distinct()
...
```{r}
tab2 <- rbind(short_meta,short_meta_low) %>%
  distinct() 
